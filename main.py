...
def deliver_packages(truck):
    current_location = 0 #4001 South 700 East HUB
    total_distance = 0
    delivery_time = truck.depart_time

    # Update packages to en route once it departs hub
    for package_id in truck.packages:
        package = hash_table.lookup(package_id)
        package.truck_number = truck.truck_number
        package.update_status("En route", depart_time=truck.depart_time)

    # separate packages that need to be delivered by 10:30 AM
    urgent_packages = []
    regular_packages = []
...
# Helper method for CLI to prevent repetition
def check_package_status(package_id, time_to_check):
        ...
        if package:
            ...
            if time_to_check < depart_time:
                ...
                status = f"Delivered at {package.delivery_time} to {package.delivery_address} by Truck {package.truck_number}"
            else:
    ...
    while True:
        ...
        try:
            ...
            if choice == 1:
                ...
                if package_choice == 1:
                    package_id = int(input("Enter the package ID to view details: "))
                ...
                # Option 2: View Truck 1 Total Distance
